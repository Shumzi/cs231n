...
# Recall that the Leaky ReLU nonlinearity computes  𝑓(𝑥)=max(𝛼𝑥,𝑥)  for some fixed constant  𝛼 ; for the LeakyReLU nonlinearities in the architecture above we set  𝛼=0.01 .
    model = nn.Sequential(
        Flatten(),
        nn.Linear(784, 256), # default bias=True
...
def generator_loss(logits_fake):
    ...
    loss = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    max_false_labels = torch.ones_like(logits_fake).type(dtype)
    loss = bce_loss(logits_fake, max_false_labels)
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = 0.5 * (real_loss.mean() + fake_loss.mean())
